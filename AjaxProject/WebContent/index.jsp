...
<body>
	...
	<script>
		const ajaxTest1 = () => {
			...
			// 2. xhr 객체를 설정하기 -> 전송방식(get/post), 요청을 보내는 주소 / [동기식,비동기식]
	...
	<pre>
		...
		* 부수적인 속성
		...
		- dataType : 서버에서 response로 넘어오는 데이터의 데이터 자료형설정. 값이 없다면 알아서 판단함★
		             ...
		             script : javascript 및 일반 STRING형태의데이터
		             html : html태그 자체를 return하는 방식
		             text : String 데이터
		- accept : 파라미터의 타입 설정(사용자 특화된 파리미터 타입 설정 가능)
		- beforeSend : ajax요청을 하기 전 실행되는 이벤트함수를 정의(데이터 가공, header관련 설정을함)
		...
		- global : 기본 이벤트 사용 여부(ajaxStrart, ajaxStop) => 선처리 작업시 사용
	...
	<h4>4. 응답데이터로 여러개의 객체들이 담겨있는는 ArrayList 받기</h4>
	...
	<script>
		function sendFile(){
			//파일 전송시 FormData객체를 생성해서 FormData내부에 파일을 넣어준후, 요청을 보내야함.
